...
export default function FeaturedProject({ project, index }: FeaturedProjectProps) {
  return (
    ...
    >
      <div
        className={cn(
          "relative overflow-hidden rounded-3xl border border-[var(--glass-border)] bg-[var(--glass)] backdrop-blur-xl",
          "transition-all duration-500",
          "hover:border-[var(--glass-border-hover)] hover:shadow-[var(--shadow-xl)]"
        )}
      >
        <div className="grid gap-0 lg:grid-cols-[1.1fr_1fr]">
          ...
          <div className="relative overflow-hidden">
            ...
            >
              ...
              <div className="absolute left-5 top-5">
                <span className="inline-flex items-center rounded-full border border-white/20 bg-black/40 px-3 py-1 text-xs font-semibold text-white shadow-sm backdrop-blur-md">
                  Featured
                </span>
              </div>

              {/* Decorative circles */}
              <div className="absolute bottom-8 right-8 size-24 rounded-full border border-white/15" />
          ...
          <div className="flex flex-col gap-6 p-7 sm:p-8 lg:p-10">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex size-9 shrink-0 items-center justify-center rounded-full border border-[var(--glass-border)] bg-[var(--surface-overlay)] transition-all duration-300 group-hover:border-[var(--primary)] group-hover:bg-[var(--primary-muted)]">
                <ArrowUpRight
            ...
            <div className="flex flex-wrap gap-2">
              {project.techStack.map((tech) => (
                <span
                  key={tech}
                  className="rounded-lg border border-[var(--glass-border)] bg-[var(--surface-overlay)] px-3 py-1 text-xs font-medium text-[var(--foreground-secondary)] transition-colors duration-300 group-hover:border-[var(--glass-border-hover)] group-hover:bg-[var(--surface-sunken)]"
                >
                  {tech}
                </span>
              ))}
